...
def kmeans(k, data, nr_iter = 100, workers=1):
    ...
    for j in range(nr_iter):
        ...
        splitted_data = np.array_split(data, workers)

        res = p.starmap(findCentroids, [(x,centroids) for x in splitted_data])
        res = np.transpose(res)
        c_w = res[0]

        cluster_size_w = res[1]
        variation_w = res[2]
        cluster_sizes = np.sum(cluster_size_w, axis=0)
        variation = np.sum(variation_w, axis=0)
        c = np.array(list(c_w)).reshape(-1) 
        ...
        # Recompute centroids
        c_split = np.array_split(c,workers)
        
        centroids_w = p.starmap(countClusters, [(splitted_data[i],centroids, c_split[i]) for i in range(len(splitted_data))])
        centroids = np.sum(centroids_w, axis=0)
...
def findCentroids(data, centroids):
    N = len(data)
    c = np.zeros(N)
    cluster_sizes = np.zeros(len(centroids))
    variation = np.zeros(len(centroids))
    for i in range(N):
...
def countClusters(data, centroids, c):
    N = len(data)
    for i in range(N):
        centroids[int(c[i])] += data[i]
    return centroids 
